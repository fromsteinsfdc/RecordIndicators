...
import saveIndicator from '@salesforce/apex/RecordIndicatorsController.saveIndicator';
...
export default class IndicatorItemBuilder extends LightningElement {
    @api indicator = {
    ...
    showSpinner = false;
    activeVariantTabIndex = 0;
    // apiNameRegexPattern = REGEX.API_NAME_PATTERN;
    // messageWhenPatternMismatch = REGEX.API_MISMATCH_MESSAGE;

    get activeVariant() {
        return this.itemVariants[this.activeVariantTabIndex];
    }

    get tabs() {
        return this.itemVariants.map((variant, index) => {
            return {
                index,
                variant,
                tabLabel: variant.Label || 'New Variant',
                tabClass: 'slds-vertical-tabs__nav-item' + (index == this.activeVariantTabIndex ? ' slds-is-active' : '')
            }
        })
    }

    get missingObjectName() {
        return !this.indicator.Object_Name__c;
    }
    ...
    connectedCallback() {
        ...
        if (this.itemVariants.length === 0) {
            this.addNewVariant('Default', false);
        }
    }
    ...
    handleIndicatorPropertyChange(event) {
        console.log(`in handleIndicatorPropertyChange`);
        this.indicator[event.target.dataset.property] = getChangedValue(event);

        // console.log(`tagName = ${event.target.tagName}`);
        // const target = event.target;
        // let propertyName = target.dataset.property;
        ...
        // }
        // console.log(`tagName = ${event.target.tagName} propertyName = ${propertyName}, value=${value}`);
        // this.indicator[propertyName] = value;
        this.updateIndicator();
    }

    handleLabelChange(event) {
        this.indicator.Label = event.target.value;
    ...
    handleApiNameChange(event) {
        if (this.checkApiNamePattern(event.target.value, event.target) || !event.target.value) {
            this.indicator.DeveloperName = event.target.value;
            this.updateIndicator();
        }
        // let name = event.target.value;
        // let meetsTest = REGEX.API_NAME_PATTERN.test(name);
        ...
        //     this.updateIndicator();
        // } else {
        //     event.target.setCustomValidity(REGEX.API_MISMATCH_MESSAGE);
        // }
    }
    ...
    handleVariantDeleteClick(event) {
        ...
        if (index <= this.activeVariantTabIndex) {
            this.activeVariantTabIndex = Math.max(this.activeVariantTabIndex - 1, 0);
        }
    }

    async handleObjectClearRequest(event) {
        let eventTarget = event.target;
        if (this.itemVariants.length > 1) {
            const result = await LightningConfirm.open({
                message: 'Changing the object name will remove any variants you\'ve defined. Proceed?',
                label: 'Confirm Clear Object',
                theme: 'warning'
            });
            if (result) {
                eventTarget.clearSelection();
                this.itemVariants = [this.itemVariants.pop()];
    ...
    /* ACTION FUNCTIONS */
    addNewVariant(label, canBeDeleted) {
        let newVariant = this.newIndicatorVariant(label, canBeDeleted);
        let insertIndex = this.itemVariants.length ? (this.itemVariants.length - 1) : 0;
        this.itemVariants.splice(insertIndex, 0, newVariant);
        this.activeVariantTabIndex = insertIndex;
        // this.activeVariantTabIndex = Math.max(0, this.itemVariants.length - 2); // If there's more than 
    }

    newIndicatorVariant(label, canBeDeleted = true, iconSource = 'sldsIcon') {
        let newVariant = {
            Label: label,
            canBeDeleted,
            iconSource,
    ...
    setApiNameFromLabel() {
        if (!this.indicator.DeveloperName) {
            this.indicator.DeveloperName = this.getApiNameFromLabel(this.indicator.Label);
            // this.template.querySelector('.apiName').reportValidity();
